...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        c1 = self.conv1_1(x) # (B, 8, 176, 208, 176)
        c1 = self.conv1_2(c1) # (B, 8, 176, 208, 176)
        p1 = self.pooling(c1) # (B, 8, 88, 104, 88)

        c2 = self.conv2_1(p1) # (B, 16, 88, 104, 88)
        c2 = self.conv2_2(c2) # (B, 16, 88, 104, 88)
        p2 = self.pooling(c2) # (B, 16, 44, 54, 44)

        c3 = self.conv3_1(p2) # (B, 32, 44, 54, 44)
        c3 = self.conv3_2(c3) # (B, 32, 44, 54, 44)
        p3 = self.pooling(c3) # (B, 32, 22, 27, 22)

        c4 = self.conv4_1(p3) # (B, 64, 22, 27, 22)
        c4 = self.conv4_2(c4) # (B, 64, 22, 27, 22)
        p4 = self.pooling(c4) # (B, 64, 11, 13, 11)

        c5 = self.conv5_1(p4)  # (B, 128, 11, 13, 11)
        out = self.conv5_2(c5)  # (B, 256, 11, 13, 11)
...
class IEM(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.pooling0 = nn.MaxPool3d(kernel_size=16)
        self.pooling1 = nn.MaxPool3d(kernel_size=8)
        self.pooling2 = nn.MaxPool3d(kernel_size=4)
...
class Age_predictor(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = nn.Sequential(
            nn.AvgPool3d((11, 13, 11)),
            nn.Dropout(0.5),
            nn.Conv3d(256, 33, padding=0, kernel_size=1)
        )
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.out = nn.Sequential(
            spectral_norm(nn.Conv3d(4, 1, kernel_size=1, stride=1, bias=False)))


    def forward(self, x, c4, c3, c2, c1, style):

        u0 = self.conv1(x, style)  # (B, 256, 11, 13, 11)
        u1 = nn.Upsample(scale_factor=2, mode='trilinear').cuda()(u0)  # (B, 256, 22, 27, 22)
        u1 = self.conv1_2(u1, c4, style)  # (B, (256+128), 22, 27, 22)

        u1 = self.conv2(u1) # (B, 128, 22, 27, 22)
        u2 = nn.Upsample(scale_factor=2, mode='trilinear').cuda()(u1)  # (B, 128, 45, 54, 45)
        u2 = self.conv2_2(u2, c3, style)  # (B, (128+64), 45, 54, 45)

        u2 = self.conv3(u2)  # (B, 64, 45, 54, 45)
        u3 = nn.Upsample(scale_factor=2, mode='trilinear').cuda()(u2)  # (B, 64, 91, 109, 91)
        u3 = self.conv3_2(u3, c2, style)  # (B, (64+32), 91, 109, 91)

        u3 = self.conv4(u3)  # (B, 32, 91, 109, 91)
        u4 = nn.Upsample(scale_factor=2, mode='trilinear').cuda()(u3)  # (B, 32, 182, 218, 182)
        u4 = self.conv4_2(u4, c1, style)  # (B, (32+16), 182, 218, 182)

        u4 = self.conv5(u4)  # (B, 8, 182, 218, 182)
        out = self.out(u4)  # (B, 1, 182, 218, 182)
...
class Bottle_CBN(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.model = spectral_norm(nn.Conv3d(in_channel, out_channel, kernel_size=3, stride=1, padding=1, bias=False))
...
class Skip_CBN(nn.Module):
    def __init__(self, channel):
        ...
        self.model1 = spectral_norm(nn.Conv3d(channel, channel, kernel_size=3, stride=1, padding=1, bias=False))
        self.cbn1 = ConditionalBatchNorm(channel)
        self.lrelu1 = nn.PReLU()

        self.model2 = spectral_norm(nn.Conv3d(channel, channel, kernel_size=3, stride=1, padding=1, bias=False))
...
class Conv_CBN_Dis(nn.Module):
    def __init__(self, in_c, out_c, kernel_size, stride):
        ...
        self.conv = spectral_norm(nn.Conv3d(in_c, out_c, kernel_size, stride, padding=1, bias=False))
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.out = nn.Sequential(spectral_norm(nn.Conv3d(256, 1, kernel_size=4, stride=1, padding=1)))
